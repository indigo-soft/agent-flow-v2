...
- ❌ Немає code review перед мерджем (усі коміти прямо у main)
...
5. **`chore/*`** — технічні роботи(maintenance: deps, config тощо)
    - Створюється від `main`
    - Мерджиться назад у `main` через PR
    - Видаляється після мерджу
...
Іноді треба закомітити незакінчену роботу (наприклад, зробити резервну копію перед тим як піти):
